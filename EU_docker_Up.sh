...
echo "[[ \$(pgrep -laf ^[/d]*bash.*EU_docker_AutoUp | awk -F, '{a[\$2]++}END{for (i in a) print i"'" "'"a[i]}') -le 2 ]] && " >/root/EU_docker_AutoUp.sh
echo '	until [[ -z $(docker ps -a | egrep "Created|Exited") ]]' >>/root/EU_docker_AutoUp.sh
echo '		do docker start $(docker ps -a | egrep "Created|Exited" | awk '"'{print \$1}'); done" >>/root/EU_docker_AutoUp.sh
